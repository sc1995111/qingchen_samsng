define(["jquery", "jquery-cookie"], function($){
        ...
        $.ajax({
            ...
            success:function(obj){
                for(var i = 0; i < obj.length; i++){
                    ...
                    <a class="ul_a" href="list.html" target="_blank">${obj[i].titles}</a>
    ...
    //下拉菜单显示隐藏
    function tab(){
        $(".titlenext").on("mouseenter", ".ul1 .ul1_li", function(){
            // alert()
            $(this).find($(".nav1_class")).stop(true).animate({
                height:230
            }, 400).css("display","block");
        })
        $(".titlenext").on("mouseleave", ".ul1 .ul1_li", function(){
            $(this).find($(".nav1_class")).stop(true).animate({
                height:0
    ...
    function content_2(){
        ...
        $.ajax({
            type:"get",
            url:"../data/goods_02.json",
            success:function(obj){
                
                for(var i = 0; i < obj.length; i++){
                    var node = $(`
                <li class="content_2_li" id="content_2_li${i}">
                    <a href="#"><img src="${obj[i].img}" alt=""></a>
                    <span class="title">${obj[i].title}</span><br>
                    <span class="details">${obj[i].details}</span><br>
                    <span class="price">${obj[i].price}</span>
                    <del class="overdue">${obj[i].overdue}</del>
                </li>
                `);
                node.appendTo($(".content_2 .content_2_ul"));
                }
            },
            error:function(msg){
                console.log(msg);
    ...
    function tas1(){
        $.ajax({
            ...
            success:function(obj){
                for(var i = 0; i < obj.length; i++){
                    var node1 = $(`
                    <li class="content_ul_li2" id="content_ul_li2${i}">
                    <a href="#">${obj[i].title}</a>
                    <ul class="content_ul_li_ul2" id = "ul2${i}">
                        </ul>
                    </li>
                    `);
                    node1.appendTo(".contentlist2 .content_ul2");
                    var child = obj[i].childs;
                    for(var j = 0; j < child.length; j++){
                        var node2 = $(`
                        <li class="content_list_li2" id="li2${j}">
                                <a href="#"><img src="${child[j].img}" alt=""></a>
                                <span class="title2">${child[j].titles}</span>
                                <span class="details2">${child[j].details}</span></br>
                                <span class="price2">${child[j].price}</span>
                                <del class="overdue2">${child[j].overdue}</del>
                            </li>
                        `);
                        node2.appendTo(node1.find(".content_ul_li_ul2"));
                    }
                }
            },
            error:function(msg){
    ...
    function tas2(){
        var content_ul = $(".content_ul2")
        var goods_right = $(".content_right2");
        var goods_left = $(".content_left2");
        var goods_ul = $(".content_ul2");
        var items = null;
        var inow = 0;
        //向右运动
        goods_right.click(function(){
        ...
        //点击 li 进行转换
        goods_ul.on("click", ".content_ul_li2", function(){
            clearInterval(items);
            inow = $(this).index();
            tab();
            items = setInterval(function(){
        ...
        //渐入函数
        function tab(){
            var ulbtn = $(".content_ul2").find($(".content_ul_li2 .content_ul_li_ul2"));
            var libtn = $(".content_ul2").find($(".content_ul_li2"));
            ulbtn.css("opacity", 0.2).hide().eq(inow).show().animate({
                opacity:1   
            }, 800, function(){
                if(inow >= libtn.size() - 1){
        ...
        content_ul.on("mouseenter", ".content_ul_li_ul2", function(){
            clearInterval(items)
        })
        content_ul.on("mouseleave", ".content_ul_li_ul2", function(){
            items = setInterval(function(){
                inow++;
                tab();
            }, 2000)
